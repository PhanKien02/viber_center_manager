...
const SidebarContent = ({ centerSlug, branchSlug, pathname }: SidebarProps) => {
  ...
  const navItems = [
    {
      label: "Tổng quan",
      href: `/app/${centerSlug}/dashboard/${branchSlug}`,
      icon: LayoutDashboard,
    },
    {
      label: "Khóa học",
      href: `/app/${centerSlug}/dashboard/${branchSlug}/courses`,
      icon: BookOpen,
    },
    {
      label: "Lớp học",
      href: `/app/${centerSlug}/dashboard/${branchSlug}/classes`,
      icon: Users,
    },
    {
      label: "Học sinh",
      href: `/app/${centerSlug}/dashboard/${branchSlug}/students`,
      icon: GraduationCap,
    },
    {
      label: "Lịch học/dạy",
      href: `/app/${centerSlug}/dashboard/${branchSlug}/schedule`,
      icon: Calendar,
    },
    {
      label: "Học phí",
      href: `/app/${centerSlug}/dashboard/${branchSlug}/tuition`,
      icon: CreditCard,
    },
    {
      label: "Tin nhắn",
      href: `/app/${centerSlug}/dashboard/${branchSlug}/messaging`,
      icon: MessageSquare,
    },
  ];
    ...
    <div className="flex flex-col h-full">
      <div className="p-6">
        ...
        <div className="mb-8 p-4 rounded-2xl bg-white/10 backdrop-blur-sm border border-white/20">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] text-white/60 mb-1 uppercase font-bold tracking-wider">
                Chi nhánh hiện tại
              </p>
              <p className="text-sm font-bold capitalize text-white line-clamp-1">
        ...
        <nav className="space-y-1.5">
          <div className="pb-2 text-[10px] font-bold uppercase tracking-widest text-white/40 px-3">
            Hệ thống
          </div>
          <Link
          ...
          <div className="pt-6 pb-2 text-[10px] font-bold uppercase tracking-widest text-white/40 px-3">
            Quản lý cơ sở
          </div>
          {navItems.map((item) => (
            <Link
...
export default function BranchLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex">
      {/* Desktop Sidebar */}
      <aside className="w-72 bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 text-white hidden lg:flex flex-col sticky top-0 h-screen shrink-0 shadow-2xl">
        <SidebarContent
      ...
      <>
        {isMobileMenuOpen && (
          <>
            ...
            />
            <aside className="fixed left-0 top-0 bottom-0 w-72 bg-linear-to-br from-slate-900 via-slate-800 to-slate-900 text-white z-50 lg:hidden overflow-y-auto shadow-2xl animate-in slide-in-from-left duration-300">
        ...
        <header className="h-16 lg:h-20 bg-white dark:bg-slate-900 border-b dark:border-slate-800 sticky top-0 z-30 px-4 lg:px-8 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] lg:text-xs text-secondary dark:text-slate-400 italic">
                Hệ thống của {centerSlug.toUpperCase()}
              </p>
            </div>
          </div>
            ...
            <div className="relative">
              ...
              >
                <div className="text-right hidden sm:block">
                  <p className="text-xs font-bold group-hover:text-primary transition-colors dark:text-white">
                    Quản trị cơ sở
                  </p>
              ...
              <>
                {isProfileOpen && (
                  <>
                    ...
                    />
                    <div className="absolute right-0 mt-3 w-64 bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-700 p-2 z-20 animate-in fade-in zoom-in-95 duration-200">
                      ...
                      <div className="space-y-1">
                        <button
                          onClick={() =>
                            router.push(`/app/${centerSlug}/profile`)
                          }
                        ...
                        <button
                          onClick={() =>
                            router.push(`/app/${centerSlug}/settings`)
                          }
                          className="w-full flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 hover:text-primary transition-all text-sm font-bold"
                        >
                          <Settings className="w-4 h-4" />
